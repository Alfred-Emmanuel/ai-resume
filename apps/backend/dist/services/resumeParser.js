...
export class ResumeParserService {
     ...
     */
    async parseResume(extractedText) {
    ...
    parseContactInfo(text, lines) {
        const contact = {};
        // Email pattern - improved to avoid concatenation with URLs
        const emailMatch = text.match(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}(?![a-zA-Z])/g);
        if (emailMatch) {
    ...
    parseExperience(text, lines) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (experienceKeywords.some((keyword) => line === keyword ||
                    ...
                    while (j < lines.length &&
                        ...
                        if (lines[j].trim()) {
                            if (lines[j].startsWith("•") ||
                                lines[j].startsWith("-") ||
                                lines[j].startsWith("*")) {
                                entry.achievements.push(lines[j].replace(/^[•\-*]\s*/, "").trim());
                            }
                            else {
    ...
    parseSkills(text, lines) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (skillsKeywords.some((keyword) => line.includes(keyword)) &&
                (line.length < 50 ||
                    line.match(/^(skills|technical skills|technologies|competencies|expertise)$/))) {
                startIndex = i;
        ...
        for (let i = startIndex + 1; i < lines.length; i++) {
            ...
            if (line.includes(":")) {
                ...
                const skillItems = line
                    ...
                    !skill.toLowerCase().includes("reading")); // Filter out obvious non-skills
    ...
    parseCertifications(text, lines) {
        ...
        for (let i = startIndex + 1; i < lines.length; i++) {
            ...
            if (line.trim()) {
                ...
                const issuerMatch = line.match(/^(.+?)\s*-\s*(.+?)\s*(\d{4}|\d{1,2}\/\d{4})/);
                if (issuerMatch) {
    ...
    parseProjects(text, lines) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (projectKeywords.some((keyword) => line === keyword ||
                line.startsWith(keyword + " ") ||
                line.startsWith(keyword + "\n"))) {
                startIndex = i;
